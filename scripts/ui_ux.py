...
class QA_TestApp:
    ...
    def __init__(self, root):
        ...
        self.test_log = tk.Text(self.root, height=10, state='disabled', bg='#f0f0f0')
    ...
    def create_widgets(self):
        ...
        self.create_test_button("2. 느린 로딩 시 사용자 불안 최소화 테스트", self.test_loading_anxiety)
        self.create_test_button("3. 재생, 일시정지, 배속 기능 테스트", self.test_player_controls)
        self.create_test_button("4. 콘텐츠 탐색 직관성 테스트", self.test_content_search)
        self.create_test_button("5. 모바일 환경 UI 테스트 (시뮬레이션)", self.test_mobile_ui)
        self.create_test_button("6. 중간 종료 후 이어보기 기능 테스트", self.test_continue_watching)
        self.create_test_button("7. 강의 완료/퀴즈 결과 알림 테스트", self.test_quiz_notification)
        self.create_test_button("8. 글씨 크기와 대비 WCAG 기준 충족 테스트", self.test_wcag_compliance)
        self.create_test_button("9. 불편사항 쉽게 신고 기능 테스트", self.test_report_issue)
        self.create_test_button("10. 자막이 강의 오디오와 정확히 맞는지 테스트", self.test_subtitle_sync)

    def create_test_button(self, text, command):
        #테스트 버튼을 생성하고 메인 프레임에 추가하는 헬퍼 함수
        btn = ttk.Button(self.main_frame, text=text, command=lambda: self.run_test(text, command))
    ...
    def test_loading_anxiety(self):
        ...
        cancel_btn = ttk.Button(loading_window, text="취소", command=lambda: loading_window.destroy())
        cancel_btn.pack()
        
        def simulate_progress():
            for i in range(101):
                if not loading_window.winfo_exists(): return
                time.sleep(0.02)
    ...
    def test_report_issue(self):
        ...
        messagebox.showinfo("테스트 결과", "'불편사항 신고' 버튼이 쉽게 찾을 수 있는 곳에 있으며, 정상적으로 작동합니다.")
